...
class VFASARSA:
    ...
    def iteration(self,k):
        ...
        while not done:
            ...
            next_action = self.eposilon_greedy(self.postion2num(next_state))
            q_s = self.FeatureVectors(self.postion2num(state),action)
            q_s_1 = self.FeatureVectors(self.postion2num(next_state),next_action)
            td_error = reward + self.gamma * q_s_1.transpose() @ self.w - q_s.transpose() @ self.w
